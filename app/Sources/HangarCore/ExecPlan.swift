...
public enum ExecPlan {
    ...
    public static func plan(_ template: [String], for entry: SearchEntry) -> Planned {
        let values = placeholders(for: entry)
        var vector: [String] = []
        // A bare `--` in the template is the caller saying the program stops
        // reading options there, which is the remedy the documentation offers.
        // Refusing past it rejected the fix while advising it.
        var optionsEnded = false
        for argument in template {
            let filled = substitute(argument, values)
            if argument == "--" { optionsEnded = true }
            if !optionsEnded, argument.hasPrefix("{"), filled.hasPrefix("-") {

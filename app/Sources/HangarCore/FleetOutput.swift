...
public enum FleetOutput {
    ...
    /// A tag value made safe to draw on a terminal.
    ///
    /// Tags are written by anyone who can tag the account, and a control
    /// character in one is not a display problem: `\r` returns the cursor to
    /// column zero so the rest of the value overwrites the row, and `ESC` opens
    /// a sequence that can move the cursor anywhere. Either one lets a tag forge
    /// a row that says something the fleet does not.
    ///
    /// Replaced rather than stripped, so the reader can see a value was altered
    /// instead of quietly reading a shortened one. Not applied to `columns`,
    /// which is the shape 0.6.1 printed and something is parsing: those bytes
    /// went out unfiltered then and nothing branches on them.
    static func display(_ value: String) -> String {
        guard value.unicodeScalars.contains(where: isControl) else { return value }
        return String(String.UnicodeScalarView(
            value.unicodeScalars.map { isControl($0) ? "\u{FFFD}" : $0 }))
    }
        ...
        for entry in order.flatMap({ buckets[$0] ?? [] }) {
            if grouped {
                ...
                if current != shown {
                    ...
                    lines.append(terminal.styled(
                        current.isEmpty ? "untagged" : display(current), .heading))
                    shown = current
                }
            }

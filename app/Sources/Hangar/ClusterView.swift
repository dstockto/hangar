...
final class ClusterView: NSView {
    ...
    func show(_ instances: [Instance], groupBy keys: [String]) {
        ...
        nodes = byGroup.keys.sorted().enumerated().map { index, key in
            ...
            return Node(
                ...
                position: seed,
                radius: 12 + min(26, sqrt(CGFloat(members.count)) * 4),
    ...
    private func step() {
        ...
        for index in nodes.indices {
            var force = CGVector(
                dx: (centre.x - nodes[index].position.x) * 0.012,
                dy: (centre.y - nodes[index].position.y) * 0.012)
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        // Hosts first, batched by state so the whole ring is a few fills rather
        // than one per host.
        ...
        // Labels last, and on a pill: drawn inline they ended up underneath the
        // next node's circle, which is exactly when a label is least readable.
        for (index, node) in nodes.enumerated() {
            let name = node.label
            let attributes: [NSAttributedString.Key: Any] = [

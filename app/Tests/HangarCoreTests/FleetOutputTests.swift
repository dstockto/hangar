...
final class FleetOutputTests: XCTestCase {
    ...
    // MARK: - Tags are drawn, not obeyed

    /// A tag carrying CR returns the cursor to column zero, so the rest of the
    /// value overwrites the row it was drawn on. In a numbered chooser that
    /// makes the number you type not the host you read.
    func testAControlCharacterCannotRedrawARow() {
        XCTAssertEqual(FleetOutput.display("real.example.com\r  9  fake.example.com"),
                       "real.example.com\u{FFFD}  9  fake.example.com")
    }

    /// ESC is the worse one: it opens a sequence that can move the cursor
    /// anywhere on the screen, not just to the start of this line.
    func testEscapeIsNeutralisedToo() {
        XCTAssertEqual(FleetOutput.display("web\u{1B}[2Kgone"), "web\u{FFFD}[2Kgone")
        XCTAssertEqual(FleetOutput.display("a\nb"), "a\u{FFFD}b")
        XCTAssertEqual(FleetOutput.display("a\u{7F}b"), "a\u{FFFD}b")
    }

    /// Replaced rather than stripped, so a reader can see the value was altered
    /// instead of quietly reading a shortened one.
    func testAnOrdinaryValueIsUntouched() {
        XCTAssertEqual(FleetOutput.display("web-1.example.com"), "web-1.example.com")
        XCTAssertEqual(FleetOutput.display("caf\u{00E9}\u{1F680}"), "caf\u{00E9}\u{1F680}")
        XCTAssertEqual(FleetOutput.display(""), "")
    }

    /// End to end: no row the terminal listing emits carries a control character.
    func testTheTerminalListingNeverEmitsAControlCharacter() {
        var hostile = Fixture.instance(["product": "p\rod", "env": "prod",
                                        "Name": "web"])
        hostile.tags["hostname"] = "real.example.com\r  9  fake.example.com"
        let text = FleetOutput.listing([SearchEntry(instance: hostile, alias: "web-1")],
                                       terminal: monochrome, grouped: true)
        for scalar in text.unicodeScalars where scalar != "\n" {
            XCTAssertFalse(scalar.value < 0x20 || scalar.value == 0x7F,
                           "control character U+\(String(scalar.value, radix: 16)) reached the row")
        }
    }

    /// The piped shape is what 0.6.1 printed and something is parsing it. Those
    /// bytes went out unfiltered then, and nothing branches on them.
    func testThePipedShapeIsStillUnfiltered() {
        var hostile = Fixture.instance(["product": "payments", "env": "prod"])
        hostile.tags["hostname"] = "real\rfake"
        let entries = [SearchEntry(instance: hostile, alias: "web-1")]
        XCTAssertTrue(FleetOutput.columns(entries).contains("real\rfake"))
        XCTAssertEqual(FleetOutput.listing(entries, terminal: .plain, grouped: true),
                       FleetOutput.columns(entries))
    }
}

...
final class ConnectDecisionTests: XCTestCase {
    ...
    /// The chooser is the one place a person reads a row and then types a number
    /// that acts on it, so a tag able to redraw the row decides which host gets
    /// the session. Nothing it emits may carry a control character.
    func testNoControlCharacterReachesTheChooser() {
        var hostile = Fixture.instance(["product": "p\rod", "env": "prod"])
        hostile.tags["hostname"] = "real.example.com\r  9  fake.example.com"
        let text = FleetOutput.numbered(
            [SearchEntry(instance: hostile, alias: "web-1")], terminal: .plain)
        for scalar in text.unicodeScalars where scalar != "\n" {
            XCTAssertFalse(scalar.value < 0x20 || scalar.value == 0x7F,
                           "U+\(String(scalar.value, radix: 16)) reached a chooser row")
        }
        // The payload is still visible, inline, on the row it belongs to.
        XCTAssertTrue(text.contains("real.example.com\u{FFFD}"))

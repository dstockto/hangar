...
final class ExecPlanTests: XCTestCase {
    ...
    /// The remedy the documentation offers has to work. Refusing past a `--` that
    /// the caller wrote rejected the fix while advising it in the same message.
    func testADoubleHyphenInTheTemplateAllowsThePlaceholder() {
        let hostile = entry("web-1", ["hostname": "-oProxyCommand=id"])
        guard case .run(let vector) = ExecPlan.plan(["ssh", "--", "{hostname}", "uptime"],
                                                    for: hostile)
        else { return XCTFail("the documented remedy was refused") }
        XCTAssertEqual(vector, ["ssh", "--", "-oProxyCommand=id", "uptime"])
    }

    /// And without it, still refused, so the guard has not simply been removed.
    func testWithoutTheDoubleHyphenItIsStillRefused() {
        let hostile = entry("web-1", ["hostname": "-oProxyCommand=id"])
        guard case .refuse = ExecPlan.plan(["ssh", "{hostname}"], for: hostile)
        else { return XCTFail("the hostile hostname planned a run") }
    }

    /// A `--` after the placeholder does not retroactively make it safe.
    func testADoubleHyphenAfterThePlaceholderDoesNotCount() {
        let hostile = entry("web-1", ["hostname": "-oProxyCommand=id"])
        guard case .refuse = ExecPlan.plan(["ssh", "{hostname}", "--"], for: hostile)
        else { return XCTFail("a trailing -- excused a leading placeholder") }
    }
